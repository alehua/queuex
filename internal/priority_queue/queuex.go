...
// 参考: https://github.com/alehua/ekit/blob/master/internal/queue/priority_queue.go
...
func (p *PriorityQueue[T]) Top() (T, error) {
	if p.isEmpty() {
		var t T
...
func NewPriorityQueue[T any](capacity int, compare Compare[T]) *PriorityQueue[T] {
	sliceCap := capacity + 1
	if capacity < 1 {
		capacity = 0
		sliceCap = 64
